...
- **`nav_churn`（业主 2026-08-09）为什么要单独一条，而不是把 `nav` 调激进**：
  ...
  （与全局间隔同值但独立声明，`CHURN_MIN_SWITCHES = 5` / `CHURN_MIN_SINCE_AD_MS = 60_000`）。
...
`src/constants/adPacing.ts` → **`MIN_AD_INTERVAL_MS = 60s`**（业主 2026-08-14 调回，嫌密度高；2026-08-08 曾降到 30s——那一版也是关广告连锁热启动 bug 变得可达的原因，修复见 §3）。
