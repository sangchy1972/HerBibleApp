...
### 3.1 先开导出（**最要紧的一步，越早越好**）

Firebase 控制台 → ⚙️ **项目设置** → **集成** → **BigQuery** → 链接

- 勾上 **Google Analytics** 数据集
- 频率选 **每日** + **串流**（串流是当天的实时表，每日是次日的完整表）

> ⚠️ **导出不回溯。** 开启之前的数据永远进不了 BigQuery。所以哪怕你这周不做分析，
> 也应该现在就把它打开。

开完之后，数据集叫 `analytics_<你的GA4属性ID>`，表是
`events_YYYYMMDD`（每日）和 `events_intraday_YYYYMMDD`（当天）。
