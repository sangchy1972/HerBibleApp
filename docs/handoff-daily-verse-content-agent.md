...
### 批次编号规则(2026-09-05 起,取代旧的"每批从 1 重开")

**day 与 id 全局连续,永不归零。** 第 N 批覆盖 day `(N−1)×60+1 … N×60`:
批次 1 = 1–60(已上线),批次 2 = 1–60(历史遗留,旧规范下交付,已按
replace 制消化),**批次 3 = 61–120**,批次 4 = 121–180,依此类推。id 恒等于
`m_`/`e_` + day 三位补零(day 61 → `m_061` / `e_061`;day 100 → `m_100`)。

为什么改:旧制下每批 id 都从 m_001 重开,而旁白音频的文件名、清单和设备缓存
都以裸 id 为键——批次 2 复用批次 1 的 id 后,音频只能整体停用(否则配错声音)。
全局连续后 id 永久唯一,音频/缓存类撞号问题从根上消失;且 app 端把新旧批拼成
一个更长的轮回(集成侧的事,你的文件只管本批 120 条),老用户会得到 120 天
不重复的内容而不是 60 天一循环。App 的循环长度读自数据,无需改代码。
...
- [ ] 每个文件恰 120 条:morning 60 + evening 60,本批 60 个 day 连续无缺无重
- [ ] `id` 与 `day/segment` 严格对应(`m_`/`e_` + day 三位补零,如 day 61 → m_061/e_061)
- [ ] day 起点 = 上一批终点 + 1(批次 3 = 61–120;写错起点集成门禁会拒收)
