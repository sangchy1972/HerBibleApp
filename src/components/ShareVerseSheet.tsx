...
export default function ShareVerseSheet({ reference, text, onClose }: Props) {
  ...
  const promptInstall = (app: AppKey) => {
    ...
    Alert.alert(
      `${cfg.label} isn't installed`,
      `Install ${cfg.label} on your phone to share verses there.`,
      [
  ...
  // Capture the chosen format, then either:
  //  • app installed → open the system share sheet titled "Share to <App>" so
  //    the brand entry is the obvious target. (True direct image-into-app
  //    requires per-app SDKs — IG Stories needs a Facebook App ID, etc.)
  //  • app not installed → prompt the user to grab it from the store.
    ...
    <View style={styles.overlay}>
      ...
      <GestureDetector gesture={pan}>
        <Animated.View
          ...
          style={[styles.sheet, sheetStyle, { paddingBottom: insets.bottom + 22 }]}
        >
          <View style={styles.handle} />
          <Text style={styles.title}>Share verse</Text>
          <Text style={styles.desc}>1. Pick a template ·  2. Choose where to share</Text>
              ...
              return (
                ...
                >
                  <View style={[
                    ...
                    <VerseCardArt format={fmt} reference={reference} text={text} width={PREVIEW_W} />
                    {isSelected && (
                  ...
                  <Text style={[styles.previewLabel, isSelected && { color: ROSE }]}>{meta.label}</Text>
          ...
          <View style={styles.offscreen} pointerEvents="none">
            {FORMATS.map(fmt => {
              ...
              return (
                ...
                >
                  <VerseCardArt format={fmt} reference={reference} text={text} width={meta.w} />
                </ViewShot>
              );
            })}
          </View>
...
const styles = StyleSheet.create({
  ...
  appBtn: { alignItems: 'center', gap: 8, paddingVertical: 8, flex: 1 },
  appGlyph: {
    width: 48, height: 48, borderRadius: 24,
    alignItems: 'center', justifyContent: 'center',
  },
  appLabel: { fontSize: 14, fontWeight: '600', color: TXT },                       // +10%
