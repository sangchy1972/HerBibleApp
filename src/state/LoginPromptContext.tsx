...
//   • one-time triggers — first highlight, day-1 open;
...
// (first_badge / first_note triggers were removed per user: they fired on the
// prayer-completion screen — a half-sheet popping over the celebration.)
...
type Trigger = 'first_highlight' | 'day1' | 'periodic';
const ONE_TIME: Trigger[] = ['first_highlight', 'day1'];
...
export function LoginPromptProvider({ children }: { children: React.ReactNode }) {
  ...
  const baselineRef = useRef({ highlights: 0 });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      baselineRef.current = { highlights: highlightCount };
  ...
  // Milestone observer — fires only on the genuine 0→1 crossing past the armed
  // baseline (so existing data on a returning user doesn't trigger it).
  useEffect(() => {
